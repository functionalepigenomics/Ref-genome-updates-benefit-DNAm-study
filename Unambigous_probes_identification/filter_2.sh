...
for file in "${list[@]}";
 ...
 echo $file
 grep -P "\+\t\S+\t50\t\d+\t50\t\S+\t\d+\t\d+\t\d+\t1\t" $file > temp1.log
 grep -P "\+\t\S+\t51\t\d+\t51\t\S+\t\d+\t\d+\t\d+\t1\t" $file > temp2.log
 #grep -P "\-\t\S+\t50\t0\t\d+\t\S+\t\d+\t\d+\t\d+\t1\t" $file > temp1.log
 #grep -P "\-\t\S+\t51\t0\t\d+\t\S+\t\d+\t\d+\t\d+\t1\t" $file > temp2.log
 cat temp1.log temp2.log > temp.log
 rm temp1.log
 rm temp2.log
 while read -r line;
